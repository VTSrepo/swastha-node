...
class UserModule {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (get_user.length==0) {
                    debug("Get User Login Details", get_user.length );
                    user_data=null
                    if (read_connection) {
                        await userDao.releaseReadConnection(read_connection);
                    }
                    var response = { user: null,menus:null,access_menu: null };
            ...
            try {
               ...
               if(data.hasOwnProperty('user_id')) {
                    ...
                    if(get_user_data.hasOwnProperty('status')) {
                        ...
                        user_user = await categories_data_to_schema_user_data_to_changepassword(data, get_user_data, date);
                        debug("Get User --->", user_user );
                        user_data = await userDao.updateUser(read_connection, user_user, data.user_id);
                        if (read_connection) {
                            await userDao.releaseReadConnection(read_connection);
                        }
    ...
    GetUserPassword(org_id,branch_id,user_id, query) {
        return new Promise(async(resolve, reject) => {
            var userDao = new UserDao();
            var connection = null;
            var get_users;
            try {
                connection = await userDao.getReadConnection();
                    ...
                    get_users = await userDao.getUserPassword(connection, user_id);
    ...
    getMachineLoginDetail(data,  query) {
        return new Promise(async (resolve, reject) => {
            var userDao = new UserDao();
            var read_connection = null;
            var get_user; 
            ...
            try {
                ...
                read_connection = await userDao.getReadConnection();
                get_user = await userDao.getMachineLogin(read_connection, data.machine_name);
...
function categories_data_to_schema_user_data_to_create(connection, data, date){
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if(data.hasOwnProperty('pf_start_date') && data.pf_start_date!='' && data.pf_start_date!=null) {                  
...
function categories_data_to_schema_user_data_to_changepassword(data, get_user_data, date) {
    return new Promise(async(resolve, reject) => {
        
        try {
            var user_data = {
               
                pwd: data.hasOwnProperty('pwd')?data.pwd:get_user_data.pwd,
                updated_by: data.hasOwnProperty('user_id')?data.user_id:get_user_data.updated_by, 
                updated_date: date
            }
            return resolve(user_data)
