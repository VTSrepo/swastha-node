...
class BillingModule {
    ...
    createBilling(data,  query) {
        return new Promise(async (resolve, reject) => {
           ...
           // var updated_date = moment(today).format("YYYY-MM-DD HH:MM:SS");
            var eod_date;
            try {
    ...
    patient360report(org_id,branch_id,patient_id,visit_in_date,visit_no, query) {
        return new Promise(async(resolve, reject) => {
            var billingDao = new BillingDao();
            var connection = null;
            var today = new Date();
            var date = moment(today).utc().format("YYYY-MM-DD HH:mm:ss");
            var invoice_header;
            var visit_date = moment(visit_in_date).format("YYYY-MM-DD");
           
            //var bu_id='("DIALY","LAB")';
           //var bu_id='("PHARM")';
            try {
                connection = await billingDao.getReadConnection();
                invoice_header = await billingDao.getConsultingheader(connection, org_id,branch_id, patient_id,visit_date,visit_no, query);
                if(invoice_header.hasOwnProperty('status') && invoice_header.status == 404) {
                    if (connection) {
                        await billingDao.releaseReadConnection(connection);
                    }
                 ...
                 var heading_list=[];
                 var headings = await billingDao.getPetGrpByHeading(connection, org_id, branch_id, patient_id,visit_no, query)
                ...
                if(headings.length>0) {
                    ...
                    for(var i in headings){
                        var subheading_list=[];
                        var sub_headings = await billingDao.petconsultmetadatabyheading(connection, org_id, branch_id, patient_id,visit_no,headings[i].heading, query)
                        if(sub_headings.length>0) {
                            for(var j in sub_headings){
                        
                            var details=  await billingDao.getPetConsultdetail(connection, org_id, branch_id, patient_id,visit_no,headings[i].heading,sub_headings[j].sub_heading, query) 
                ...
                invoice_header['independent_tab'] = heading_list;
                   
                   
                    // To get idiagnostic input details (D0004).
                   var diagnostic_input = await billingDao.getDiagnostic(connection,  org_id,branch_id, patient_id,visit_date,visit_no, query);
                   invoice_header['diagnostic_input'] = diagnostic_input;

                    // To get treatment deails of PET (D0004).
                    var treatment_input = await billingDao.getTreatments(connection,  org_id,branch_id, patient_id,visit_date,visit_no, query);
                    invoice_header['treatment_input'] = treatment_input;
     

                    // To get Dialysis notes
                    var dialysis_notes = await billingDao.getDialysisnotes(connection,  org_id,branch_id, patient_id,visit_date,visit_no, query);
                    invoice_header['dialysis_notes'] = dialysis_notes;
                    
                    // To get medicine prescribed in last visit 
                    var medicine_lastvisit = await billingDao.getMedicinelastvisit(connection,  org_id,branch_id, patient_id,visit_date,visit_no, query);
                    invoice_header ['medicine_lastvisit']= medicine_lastvisit;
                   
                    // To get test details of last visit
                    var test_lastvisit = await billingDao.getTestlastvisit(connection,  org_id,branch_id, patient_id,visit_date,visit_no, query);
                    invoice_header ['test_lastvisit']= test_lastvisit;
                    
                    
                    //To get health details from health param table for last visit date
                    var health_lastvisit = await billingDao.getHealthlastvisit(connection,  org_id,branch_id, patient_id,visit_date,visit_no, query);
                    invoice_header ['health_lastvisit']= health_lastvisit;
                    debug("Header Reult", invoice_header)
                    if (connection) {
                        await billingDao.releaseReadConnection(connection);
                    }
                    return resolve(invoice_header)
 ...
 //Report
 invoiceproductreport(org_id,branch_id,start_date,end_date, query) {
    return new Promise(async(resolve, reject) => {
        var billingDao = new BillingDao();
        var connection = null;
        debug("start_date", start_date, "end_date", end_date)
        ...
        try {
            connection = await billingDao.getReadConnection();
            invoice_products = await billingDao.invoiceproductreport(connection, org_id, branch_id, from_date, to_date, query);
    ...
    //Report
 invoicepaymentreport(org_id,branch_id,start_date,end_date, query) {
    return new Promise(async(resolve, reject) => {
        var billingDao = new BillingDao();
        var connection = null;
        debug("start_date", start_date, "end_date", end_date)
        ...
        try {
            connection = await billingDao.getReadConnection();
            invoice_payments = await billingDao.invoicepaymentreport(connection, org_id, branch_id, from_date, to_date, query);
            if(invoice_payments.hasOwnProperty('status') && invoice_payments.status == 404) {
                if (connection) {
                    await billingDao.releaseReadConnection(connection);
                }
                return resolve(invoice_payments);
            }
            else{
                var total_size = invoice_payments.length;
                var page_size = invoice_payments.length//query.skip ? query.skip : total_size;
                var result_size = invoice_payments.length//strLimit;
                console.log("Totalsize :", total_size);
                var summary = {
                    filteredsize: page_size, resultsize: result_size, totalsize: total_size
                };
                var res = {
                    summary, results: invoice_payments
                }
                if (connection) {
                    await billingDao.releaseReadConnection(connection);
                }
    ...
    getKRCCollectionWiseReportList(org_id,branch_id,start_date,end_date, query) {
        return new Promise(async(resolve, reject) => {
            var billingDao = new BillingDao();
            var connection = null;
            var from_date = moment(start_date).format("YYYY-MM-DD");
            ...
            try {
                connection = await billingDao.getReadConnection();
                collectionwiserpt = await billingDao.getKRCRPTCollectionWiseSummaryList(connection, org_id, branch_id, from_date, to_date, query);
                if(collectionwiserpt.hasOwnProperty('status') && collectionwiserpt.status == 404) {
                    if (connection) {
                        await billingDao.releaseReadConnection(connection);
                    }
                    ...
                    for(var i in collectionwiserpt) {
                        var invoice_date = collectionwiserpt[i].inv_date;
                        var collectionwise_txn = await billingDao.getKRCRPTCollectionWiseTransactionList(connection, org_id, branch_id, query, invoice_date);
                        if(collectionwise_txn == null) {
                            var empty_array = [];
                            collectionwiserpt[i]["details"] = empty_array;
                        }
    ...
    getStockRegisterReportList(org_id,branch_id,start_date,end_date, query) {
        return new Promise(async(resolve, reject) => {
            ...
            try {
                connection = await billingDao.getReadConnection();
                if(query.filter.hasOwnProperty('product_id')) {
                    patient_types = await billingDao.getStockRegisterByProductReportList(connection, org_id, branch_id, from_date, to_date, query.filter.product_id, query);
                }else{
...
// net amount =0 inv_status =F other wise P
        ...
        try {
            ...
            for(var i in data.invoice_details) {
               ...
               /*  if(invoice_details.hasOwnProperty('other_charge1')) {
                ...
                if(invoice_details.bu_id == 'DIALY'){
                    buid_check="Y";
                }
                ...
                //Shan Test
                //select count(*) from billing detail where org= org id and branch id = branch id and invoice number = new invoice number and bu_id = "DIALY"
//If count is >=1  then call
                debug("shan detail******>", billing_detail_data );
            ...
            if(get_patient_data.patient_type != 'N' &&  buid_check =='Y') {
            ...
            debug("shan Header******>", update_billing_header);
...
function categories_data_to_schema_billing_payment_data_create(connection, data, get_payment_data, get_billing_data, get_billing_head, date,eod_date) {
    return new Promise(async(resolve, reject) => {
        try {
            var billingDao = new BillingDao();
            var patientDao = new PatientDao();
            var payment_num, inv_status, bu_patient_inv_status, patient_inv_status;
            var net_paid = 0;
            var net_balance = 0;
            var bu_patient_amt_paid = 0;
            var bu_patient_amt_bal = 0;
            var net_inv_paid = 0;
            var net_inv_balance = 0;
            var net_patient_inv_balance=0;
            var net_patient_inv_paid=0;
            ...
            var billing_payment_data = {
                ...
                payment_source:(data.hasOwnProperty('payment_source'))?data.payment_source: null, 
                updated_by: (data.hasOwnProperty('user_id'))?data.user_id: null, 
                updated_date: date, 
                created_by: (data.hasOwnProperty('user_id'))?data.user_id: null, 
                created_date: date
            ...
            bu_patient_amt_paid =   get_billing_data.bu_patient_amt_paid + data.payment_amount;
            bu_patient_amt_bal  =   get_billing_data.bu_patient_amt_bal -  data.payment_amount;
            ...
            //bu_patient_amt
            if(get_billing_data.bu_patient_amt == bu_patient_amt_paid) {
                bu_patient_inv_status = 'F';
            }
            else{
                bu_patient_inv_status = 'P';
            }
            ...
            var update_billing_bu_detail = {
                ...
                bu_patient_amt_paid:bu_patient_amt_paid,
                bu_patient_amt_bal: bu_patient_amt_bal,
                bu_patient_inv_status:bu_patient_inv_status,
                updated_date: date
            }
            ...
            net_patient_inv_paid = get_billing_head.net_patient_inv_paid + data.payment_amount;
            net_patient_inv_balance = get_billing_head.net_patient_inv_balance - data.payment_amount;
           ...
           //net_patient_inv_amount
            if(get_billing_head.net_patient_inv_amount == net_patient_inv_paid) {
                patient_inv_status = 'F';
            }
            else{
                patient_inv_status = 'P';
            }
            ...
            var update_billing_head = {
                ...
                inv_status: inv_status,
                net_patient_inv_paid: net_patient_inv_paid,
                net_patient_inv_balance: net_patient_inv_balance,
                patient_inv_status: patient_inv_status,
                updated_date: date
            }
