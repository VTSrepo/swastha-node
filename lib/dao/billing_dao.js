...
class BillingDao extends BaseDao {
        ...
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                if(query.filter.inv_status.toUpperCase() == 'A') {
                    ...
                    if(query.filter.patient_id) {
                        custQuery = `SELECT *,(total_base_amt+total_charges) as gross_amt FROM ${process.env.WRITE_DB_DATABASE}.Billing_Header WHERE branch_id='${branch_id}' AND patient_id='${query.filter.patient_id}' AND patient_inv_status='${inv_status}'`;
                    }
                    else if(query.filter.doctor_id) {
        ...
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                if(query.filter.inv_status.toUpperCase() == 'A') {
                    ...
                    if(query.filter.patient_id) {
                        custQuery = `SELECT COUNT(*) as count FROM ${process.env.WRITE_DB_DATABASE}.Billing_Header WHERE branch_id='${branch_id}' AND patient_id='${query.filter.patient_id}' AND patient_inv_status='${inv_status}'`;
                    }
                    else if(query.filter.doctor_id) {
        ...
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                var custQuery = `Select org_id,branch_id,invoice_no,bu_id, sum(product_cost * product_qty) base_cost, 
                            ...
                            Sum(discount1) discount1 ,sum(discount2) discount2,sum(discount3) discount3,sum(gross_discount) gross_discount,sum(net_amount) net_amount,
                            sum(net_patient_amount) bu_patient_amt, sum(insurance_inv_value) bu_insurance_amt, sum(doctor_inv_value) bu_doctor_amt
                            from ${process.env.WRITE_DB_DATABASE}.Billing_Detail where org_id='${org_id}' and branch_id='${branch_id}'  
    ...
    getConsultingheader(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
               ...
               and c.visit_no=${visit_no} and c.visit_date='${visit_date}' `; 
                
              
                debug("getConsultingheader", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    debug('Sorry, Billing Not Found!.', queryres);
    ...
    getPetGrpByHeading(connection, org_id, branch_id, patient_id,visit_no, query){
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                var custQuery = `select a.heading,b.heading_seq_no from ${process.env.WRITE_DB_DATABASE}.swastha_pet_consult_detail a left join  ${process.env.WRITE_DB_DATABASE}.swastha_doctor_pet_consult_metadata b 
                ...
                group by a.heading,b.heading_seq_no 
                order by b.heading_seq_no`;
    ...
    getPetConsultdetail(connection, org_id, branch_id, patient_id,visit_no,heading,sub_heading, query) {
        return new Promise(async(resolve, reject)=> {
            try{
               ...
               /* var custQuery = `Select * from  ${process.env.WRITE_DB_DATABASE}.swastha_pet_consult_detail a
                ...
                debug("getPetConsultdetail", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var empty_array = [];
...
// 360 getDiagnostic
    getDiagnostic(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                    ...
                    var custQuery = `SELECT * FROM ${process.env.WRITE_DB_DATABASE}.swastha_pet_diagnosis 
                    where org_id='${org_id}' and branch_id='${branch_id}'  and patient_id='${patient_id}'  
                    and visit_no=${visit_no} `;
               
                
                debug("getDiagnostic", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = "";
                    return resolve(invoice_lists);
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getDiagnostic error :', error)
                return reject(err_code);
            }
        })
    }
    ...
    // 360 getTreatments
    getTreatments(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getTreatments error :', error)
                return reject(err_code);
            }
        })
    }
// 360 getDialysisnotes
    getDialysisnotes(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                    ...
                    var custQuery = `SELECT DATE_FORMAT(prescription_date,'%Y-%m-%d') prescription_date,dialysis_notes,doctor_id FROM ${process.env.WRITE_DB_DATABASE}.Patient_Consult_Dialysis_Detail 
                    where org_id='${org_id}' and branch_id='${branch_id}' and patient_id='${patient_id}' 
                    and visit_no=${visit_no} `;
               
                
                debug("getDialysisnotes", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = "";
                    return resolve(invoice_lists);
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getDialysisnotes error :', error)
                return reject(err_code);
            }
        })
    }
    // 360 getMedicinelastvisit
    getMedicinelastvisit(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                    ...
                    and a.visit_no =${visit_no}   
                    and a.org_id=b.org_id and a.medicine_id=b.product_id `;
                
                
                debug("getMedicinelastvisit", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = "";
                     return resolve(invoice_lists);
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getMedicinelastvisit error :', error)
                return reject(err_code);
            }
        })
    }
   // 360 getTestlastvisit
   getTestlastvisit(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
    return new Promise(async(resolve, reject)=> {
        try{
            if(connection == null) {
                var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
            ...
            debug("getTestlastvisit", custQuery)
            let queryres = await connection.query(custQuery);
            if(queryres.length == 0) {
                var invoice_lists = "";
                return resolve(invoice_lists);
            ...
            var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
            debug('getTestlastvisit error :', error)
            return reject(err_code);
        }
    })
}
       // 360 getTestlastvisit
       //To get health details from health param table for last visit date
   getHealthlastvisit(connection, org_id,branch_id, patient_id,visit_date,visit_no, query) {
    return new Promise(async(resolve, reject)=> {
        try{
            if(connection == null) {
                var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
            ...
            debug("getHealthlastvisit", custQuery)
            let queryres = await connection.query(custQuery);
            if(queryres.length == 0) {
                var invoice_lists = "";
                return resolve(invoice_lists);
    ...
    getPatientInvoiceDetail(connection, invoice_no, patient_id, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                var custQuery = `SELECT b.branch_name,  b.branch_address,b.branch_contact_num,b.branch_cont_pers_phone,p.patient_id, p.patient_name, p.sex, p.age ,h.inv_status,d.doctor_name 
                ...
                INNER JOIN ${process.env.WRITE_DB_DATABASE}.Branch_Master b ON b.branch_id=h.branch_id LEFT JOIN Doctor_Master d on d.doctor_id=h.doctor_id  
                WHERE h.patient_id='${patient_id}' AND  h.invoice_no='${invoice_no}'`;
    ...
    // A1 : Report Doctor Summary
    getReportDoctorSummary(connection, org_id, branch_id, from_date,to_date, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                ifnull(sum(Dialysis),0) dialysis, 
                ifnull(sum(bu_patient_amt_Dialysis),0) bu_patient_amt_dialysis,
                ifnull(sum(bu_insurance_amt_Dialysis),0) bu_insurance_amt_dialysis,
                ifnull(sum(bu_doctor_amt_Dialysis),0) bu_doctor_amt_dialysis,
                ifnull(sum(Pharmacy),0) pharmacy,
                ifnull(sum(bu_patient_amt_Pharmacy),0) bu_patient_amt_pharmacy,
                ifnull(sum(bu_insurance_amt_Pharmacy),0) bu_insurance_amt_pharmacy,
                ifnull(sum(bu_doctor_amt_Pharmacy),0) bu_doctor_amt_pharmacy,
                ifnull(sum(Lab),0) lab,
                ifnull(sum(bu_patient_amt_Lab),0) bu_patient_amt_lab,
                ifnull(sum(bu_insurance_amt_Lab),0) bu_insurance_amt_lab,
                ifnull(sum(bu_doctor_amt_Lab),0) bu_doctor_amt_lab,
                ifnull(sum(Others),0) others,
                ifnull(sum(bu_patient_amt_Others),0) bu_patient_amt_others,
                ifnull(sum(bu_insurance_amt_Others),0) bu_insurance_amt_others,
                ifnull(sum(bu_doctor_amt_Others),0) bu_doctor_amt_others,
                ...
                ifnull(sum(net_balance),0) net_balance,
                ifnull(sum(bu_patient_amt_paid),0) bu_patient_amt_paid,
                ifnull(sum(bu_patient_amt_bal),0) bu_patient_amt_bal
                 from 
                ...
                (CASE WHEN b.bu_id="DIALY" THEN b.bu_patient_amt  ELSE 0 END) "bu_patient_amt_Dialysis",
                (CASE WHEN b.bu_id="LAB" THEN b.bu_patient_amt  ELSE 0 END) "bu_patient_amt_Lab",
                (CASE WHEN b.bu_id="PHARM" THEN b.bu_patient_amt ELSE 0 END) "bu_patient_amt_Pharmacy",
                (CASE WHEN b.bu_id NOT IN ("PHARM","DIALY","LAB") THEN b.bu_patient_amt  ELSE 0 END) "bu_patient_amt_Others",
                (CASE WHEN b.bu_id="DIALY" THEN b.bu_insurance_amt  ELSE 0 END) "bu_insurance_amt_Dialysis",
                (CASE WHEN b.bu_id="LAB" THEN b.bu_insurance_amt  ELSE 0 END) "bu_insurance_amt_Lab",
                (CASE WHEN b.bu_id="PHARM" THEN b.bu_insurance_amt  ELSE 0 END) "bu_insurance_amt_Pharmacy",
                (CASE WHEN b.bu_id NOT IN ("PHARM","DIALY","LAB") THEN b.bu_insurance_amt  ELSE 0 END) "bu_insurance_amt_Others",
                (CASE WHEN b.bu_id="DIALY" THEN b.bu_doctor_amt  ELSE 0 END) "bu_doctor_amt_Dialysis",
                (CASE WHEN b.bu_id="LAB" THEN b.bu_doctor_amt  ELSE 0 END) "bu_doctor_amt_Lab",
                (CASE WHEN b.bu_id="PHARM" THEN b.bu_doctor_amt  ELSE 0 END) "bu_doctor_amt_Pharmacy",
                (CASE WHEN b.bu_id NOT IN ("PHARM","DIALY","LAB") THEN b.bu_doctor_amt  ELSE 0 END) "bu_doctor_amt_Others",
                ...
                ifnull(b.bu_patient_amt_paid,0) bu_patient_amt_paid,
                ifnull(b.bu_patient_amt_bal,0)  bu_patient_amt_bal
                ...
                debug("getReportDoctorSummary", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = [];
                    return resolve(invoice_lists);
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getReportDoctorSummary error :', error)
                return reject(err_code);
            }
        })
    }

      // A1 : Report Doctor Summary Detail
      getReportDoctorSummaryDetail(connection, org_id, branch_id, inv_date, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `
                Select DATE_FORMAT(inv_date,'%Y-%m-%d') inv_date, invoice_no,patient_name,e.ref_desc,
                ifnull(sum(Dialysis),0) dialysis, 
                ifnull(sum(bu_patient_amt_Dialysis),0) bu_patient_amt_dialysis,
                ifnull(sum(bu_insurance_amt_Dialysis),0) bu_insurance_amt_dialysis,
                ifnull(sum(bu_doctor_amt_Dialysis),0) bu_doctor_amt_dialysis,
                ...
                and a.invoice_no=b.invoice_no
                and a.patient_id = c.patient_id
                and a.inv_date ='${inv_date}' )d, ${process.env.WRITE_DB_DATABASE}.Swastha_Ref_Master e
                where d.patient_type=e.ref_code and e.ref_type="PATTYP"
                group by inv_date, invoice_no,patient_name,e.ref_desc  order by inv_date,invoice_no`;



                debug("getReportDoctorDetail", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = [];
                    return resolve(invoice_lists);
    ...
    // A1 : Report invoiceproductreport
    invoiceproductreport(connection, org_id, branch_id, from_date,to_date, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `Select DATE_FORMAT(a.inv_date,'%Y-%m-%d') inv_date, a.invoice_no,a.patient_id,c.patient_name,e.ref_desc as patient_type,b.bu_id, d.product_name, b.product_qty,b.product_value,
                (b.other_charge1 + b.other_charge2 + b.other_charge3) tot_charges,
                b.gross_discount, b.net_amount
                from ${process.env.WRITE_DB_DATABASE}.Billing_Header a, ${process.env.WRITE_DB_DATABASE}.Billing_Detail b , ${process.env.WRITE_DB_DATABASE}.Patient_Master c, ${process.env.WRITE_DB_DATABASE}.Product_Master d, 
                ${process.env.WRITE_DB_DATABASE}.Swastha_Ref_Master e
                where a.org_id='${org_id}'  and a.branch_id='${branch_id}' and a.invoice_no = b.invoice_no
                and a.patient_id=c.patient_id and b.product_id=d.product_id
                and c.patient_type = e.ref_code and e.ref_type="PATTYP" 
                and a.inv_date between '${from_date}' and '${to_date}' 
                order by a.inv_date, a.invoice_no`;

                debug("invoiceproductreport", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = [];
                    return resolve(invoice_lists);
    ...
    // A1 : Report invoicepaymentreport
    invoicepaymentreport(connection, org_id, branch_id, from_date,to_date, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `Select DATE_FORMAT(a.inv_date,'%Y-%m-%d') inv_date, a.invoice_no,c.patient_name,e.ref_desc as patient_type,b.bu_id,b.base_cost,
                (b.other_charge1 + b.other_charge2 + b.other_charge3) tot_charges,
                b.gross_inv_amount,b.gross_discount,b.net_amount,d.payment_date,f.ref_desc pay_mode,d.payment_amount,
                b.net_paid,b.net_balance,b.inv_srl_no
                from ${process.env.WRITE_DB_DATABASE}.Billing_Header a, ${process.env.WRITE_DB_DATABASE}.Billing_BU_Detail b , ${process.env.WRITE_DB_DATABASE}.Patient_Master c,  ${process.env.WRITE_DB_DATABASE}.Billing_Payment_Detail d,   
                ${process.env.WRITE_DB_DATABASE}.Swastha_Ref_Master e, ${process.env.WRITE_DB_DATABASE}.Swastha_Ref_Master f
                where a.org_id='${org_id}'  and a.branch_id='${branch_id}' and a.invoice_no = b.invoice_no
                and a.inv_date between '${from_date}' and '${to_date}'   
                and a.patient_id=c.patient_id 
                and c.patient_type = e.ref_code and e.ref_type="PATTYP" 
                and b.org_id=d.org_id and b.branch_id=d.branch_id and b.invoice_no = d.invoice_no and b.inv_srl_no=d.inv_srl_no
                and d.payment_mode=f.ref_code and f.ref_type="PAYMOD"
                union
                Select DATE_FORMAT(a.inv_date,'%Y-%m-%d') inv_date, a.invoice_no,c.patient_name,e.ref_desc as patient_type,b.bu_id,b.base_cost,
                (b.other_charge1 + b.other_charge2 + b.other_charge3) tot_charges,
                b.gross_inv_amount,b.gross_discount,b.net_amount," -- NA -- " payment_date," -- NA -- " pay_mode, " -- NA -- " payment_amount,
                b.net_paid,b.net_balance,b.inv_srl_no
                from ${process.env.WRITE_DB_DATABASE}.Billing_Header a, ${process.env.WRITE_DB_DATABASE}.Billing_BU_Detail b , ${process.env.WRITE_DB_DATABASE}.Patient_Master c,
                ${process.env.WRITE_DB_DATABASE}.Swastha_Ref_Master e
                where a.org_id='${org_id}'  and a.branch_id='${branch_id}' and a.invoice_no = b.invoice_no
                ...
                and a.patient_id=c.patient_id 
                and c.patient_type = e.ref_code and e.ref_type="PATTYP" 
                and b.invoice_no not in (select invoice_no from ${process.env.WRITE_DB_DATABASE}.Billing_Payment_Detail d where b.org_id=d.org_id 
                     and b.branch_id=d.branch_id and b.invoice_no = d.invoice_no)
                order by inv_date, invoice_no, inv_srl_no`;

                debug("invoicepaymentreport", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = [];
                    return resolve(invoice_lists);
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('invoicepaymentreport error :', error)
                return reject(err_code);
            }
        })
    }
...
// Outstanding Patient wise Transaction
    ...
    //KRC-Collection Wise Summary
    getKRCRPTCollectionWiseSummaryList(connection, org_id, branch_id, from_date, to_date, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                ifnull(sum(Other_charge),0) other_charge, ifnull(sum(gross_discount),0) gross_discount,
                ifnull(sum(concession),0) concession,
                ifnull(sum(insurance),0) insurance,
                ...
                (CASE WHEN b.bu_id NOT IN ("PHARM","DIALY","LAB") THEN b.net_amount  ELSE 0 END) "Others",
                (b.other_charge1+b.other_charge2+b.other_charge3) Other_charge, b.gross_discount, 
                b.discount1 concession, b.discount2 insurance, b.discount3 discount , 
                b.net_amount, ifnull(b.net_paid,0) net_paid, ifnull(b.net_balance,0) net_balance
                from ${process.env.WRITE_DB_DATABASE}.Billing_Header a, ${process.env.WRITE_DB_DATABASE}.Billing_BU_Detail b, ${process.env.WRITE_DB_DATABASE}.Patient_Master c
                where  a.org_id='${org_id}'  and a.branch_id='${branch_id}' and a.org_id = b.org_id and a.org_id = c.org_id 
                and a.branch_id=b.branch_id and a.branch_id = c.branch_id
                ...
                debug("getKRCRPTCollectionWiseSummaryList", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var invoice_lists = [];
                    return resolve(invoice_lists);
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getKRCRPTCollectionWiseSummaryList error :', error)
                return reject(err_code);
            }
        })
    }
    //KRC- CollectionWise Transaction by Invoice Date
    getKRCRPTCollectionWiseTransactionList(connection, org_id, branch_id, query, invoice_date) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                debug("getKRCRPTCollectionWiseTransactionList", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    return resolve(null);
                }
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getKRCRPTCollectionWiseTransactionList error :', error)
                return reject(err_code);
            }
        })
    }
//Collection Wise Summary
    ...
    getRPTPaymentWiseSummaryList(connection, org_id, branch_id, from_date, to_date, query) {
        return new Promise(async(resolve, reject)=> {
         ...
         ifnull(sum(adv_adjusted_inv_cnt),0) adv_adj_inv_cnt, ifnull(sum(adv_adjusted_amt),0) adv_adj_amt ,
         ifnull(sum(os_cnt),0) os_cnt, ifnull(sum(os_amt),0) os_amt,
         ifnull(sum(tot_inv_cnt),0) tot_inv_cnt, ifnull(sum(cash_amt+card_amt+debit_card_amt+chq_amt+upi_amt+adv_adjusted_amt+os_amt),0) total_inv_amt from 
         (
         Select a.inv_date,  b.bu_id, c.payment_mode,
         (case when c.payment_mode="S" then 1 else 0 end) "cash_inv_cnt", 
         (case when c.payment_mode="S" then c.payment_amount else 0 end) "cash_amt",
         (case when c.payment_mode="C" then 1 else 0 end) "card_inv_cnt", 
         (case when c.payment_mode="C" then c.payment_amount else 0 end) "card_amt",
         (case when c.payment_mode="D" then 1 else 0 end) "debit_card_inv_cnt", 
         (case when c.payment_mode="D" then c.payment_amount else 0 end) "debit_card_amt",
         (case when c.payment_mode="Q" then 1 else 0 end) "chq_inv_cnt", 
         (case when c.payment_mode="Q" then c.payment_amount else 0 end) "chq_amt",
         (case when c.payment_mode="U" then 1 else 0 end) "upi_inv_cnt", 
         (case when c.payment_mode="U" then c.payment_amount else 0 end) "upi_amt",
         (case when c.payment_mode="A" then 1 else 0 end) "adv_adjusted_inv_cnt", 
         (case when c.payment_mode="A" then c.payment_amount else 0 end) "adv_adjusted_amt",
...
group by inv_date,bu_id order by inv_date`;
    ...
    getStockRegisterByProductReportList(connection, org_id, branch_id, from_date,to_date,product_id, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `select b.product_name,a.trans_date,a.open_stock,a.received_stock,a.sold_stock,a.close_stock
                from Swastha_Daily_inventory a, Product_Master b 
                where a.org_id='${org_id}'  and a.branch_id='${branch_id}' and a.product_id='${product_id}' 
                  and a.org_id=b.org_id and a.product_id=b.product_id 
                  and a.trans_date between '${from_date}' and '${to_date}'  order by b.product_name,a.trans_date`;
