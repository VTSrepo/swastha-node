...
class UserDao extends BaseDao {
    ...
    getUserLogin(connection, user_id, pwd) {
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                var custQuery = `SELECT U.*,B.branch_name FROM ${process.env.WRITE_DB_DATABASE}.User_Master U  INNER JOIN  ${process.env.WRITE_DB_DATABASE}.Branch_Master B on B.branch_id=U.branch_id WHERE U.user_id='${user_id}' AND 
                U.pwd='${pwd}' AND U.user_status='Y'`;
    ...
    getUserList(connection, org_id,branch_id) {
        return new Promise(async(resolve, reject)=> {
            ...
            var custQuery = `SELECT U.user_id,U.org_id,U.branch_id,B.branch_name,U.user_name,U.mobile_no,U.home_contact_no,
            U.residence_address,U.email_id,U.aadhar_no,U.pan_no,U.passport,U.signature,U.bank_account_no,
            U.ifsc_code,U.bank_name,U.bank_address,U.attached_branch,U.pf_number,U.user_type,U.user_status,
            U.pwd,DATE_FORMAT(dob,'%Y-%m-%d') as dob,DATE_FORMAT(doj,'%Y-%m-%d') as doj ,DATE_FORMAT(pf_start_date,'%Y-%m-%d') as pf_start_date,U.pf_employee FROM ${process.env.WRITE_DB_DATABASE}.User_Master U 
            INNER JOIN  ${process.env.WRITE_DB_DATABASE}.Branch_Master B on B.branch_id=U.branch_id WHERE  U.user_status='Y' and U.org_id='${org_id}' AND  U.branch_id='${branch_id}'`;
    ...
    getUserId(connection, user_id) {
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                U.ifsc_code,U.bank_name,U.bank_address,U.attached_branch,U.pf_number,U.user_type,U.user_status,
                U.pwd,DATE_FORMAT(dob,'%Y-%m-%d') as dob,DATE_FORMAT(doj,'%Y-%m-%d') as doj,DATE_FORMAT(pf_start_date,'%Y-%m-%d') as pf_start_date,U.pf_employee FROM ${process.env.WRITE_DB_DATABASE}.User_Master U 
                INNER JOIN  ${process.env.WRITE_DB_DATABASE}.Branch_Master B on B.branch_id=U.branch_id WHERE  U.user_id='${user_id}'`;
    ...
    getUserPassword(connection, user_id) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `SELECT U.user_id,U.org_id,U.branch_id,U.pwd,B.branch_name,U.user_name,U.mobile_no,U.home_contact_no,
                U.residence_address,U.email_id,U.aadhar_no,U.pan_no,U.passport,U.signature,U.bank_account_no,
                U.ifsc_code,U.bank_name,U.bank_address,U.attached_branch,U.pf_number,U.user_type,U.user_status,
                U.pwd,DATE_FORMAT(dob,'%Y-%m-%d') as dob,DATE_FORMAT(doj,'%Y-%m-%d') as doj,DATE_FORMAT(pf_start_date,'%Y-%m-%d') as pf_start_date,U.pf_employee FROM ${process.env.WRITE_DB_DATABASE}.User_Master U 
                INNER JOIN  ${process.env.WRITE_DB_DATABASE}.Branch_Master B on B.branch_id=U.branch_id WHERE  U.user_id='${user_id}'`;
    ...
    getMachineLogin(connection, machine_name) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `SELECT * FROM ${process.env.WRITE_DB_DATABASE}.sbl_machine_mast U  WHERE U.machine_name='${machine_name}' `;
                debug("custQuery", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    var menu_lists = [];
