...
class UserAction {
    ...
    ChanagePassword(event, context) {      
        var body_data = event.body;
        var query = event.queryParameters;
        validate_data_for_create_user_data(event.body)       
        .then(function(_response) {
            debug("validate data ", _response);
            return userModule.ChangePassword(body_data,  query)
        })
        .then(function(response){
            if(response.hasOwnProperty('status') && (response.status == 404))
            context.done(null, SendResponse(401, response))
    ...
    GetUserPassword(event, context) {      
        ...
        .then(function(_response) {
            debug("validate data ", _response);
            return userModule.GetUserPassword(org_id, branch_id, user_id, query)
        })
        .then(function(response){
            if(response.hasOwnProperty('status') && (response.status == 404))
            context.done(null, SendResponse(401, response))
    ...
    getMachineLogin(event, context) {      
        var body_data = event.body;
        var query = event.queryParameters;
        validate_data_login(event.body)       
        .then(function(_response) {
            debug("validate data ", _response);
            return userModule.getMachineLoginDetail(body_data,  query)
        })
        .then(function(response){
            if(response.hasOwnProperty('status') && (response.status == 404))
            context.done(null, SendResponse(401, response))
